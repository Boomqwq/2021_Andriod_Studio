...
public class ClockView_2 extends View {
    ...
    private static final float HOUR_NEEDLE_LENGTH_RATIO = 0.32f; // 时针长度相对表盘半径的比例
    private static final float MINUTE_NEEDLE_LENGTH_RATIO = 0.45f; // 分针长度相对表盘半径的比例
    private static final float SECOND_NEEDLE_LENGTH_RATIO = 0.65f; // 秒针长度相对表盘半径的比例
    private static final float HOUR_NEEDLE_WIDTH = 14; // 时针的宽度
    private static final float MINUTE_NEEDLE_WIDTH = 10; // 分针的宽度
    ...
    public ClockView_2(Context context) {
        super(context);
        init();
    }

    public ClockView_2(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public ClockView_2(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void configWhenLayoutChanged() {
        ...
        numberPaint.setTextSize(Math.round(radius*0.18));
        ...
        for (int degree = 0; degree < FULL_CIRCLE_DEGREE; degree += UNIT_DEGREE) {
            ...
            float startY;
            if (degree % 30 == 0){
                startX = (float) (centerX + (radius * (1 - 0.1f)) * Math.cos(radians));
                startY = (float) (centerY + (radius * (1 - 0.1f)) * Math.sin(radians));
            }
            else {
                startX = (float) (centerX + (radius * (1 - 0.05f)) * Math.cos(radians));
                startY = (float) (centerY + (radius * (1 - 0.05f)) * Math.sin(radians));
            }
    ...
    private void drawTimeNeedles(Canvas canvas) {
         ...
         */
        float hour_Degree = 30*hour+0.5f*minute-90;
        float min_Degree = 6*minute+0.1f*second-90;
        float sec_Degree = 6*second+0.006f*millisecond-90;
        float hour_endX = (float) (centerX + radius * HOUR_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(hour_Degree)));
        float hour_endY = (float) (centerY + radius * HOUR_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(hour_Degree)));
        float min_endX = (float) (centerX + radius * MINUTE_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(min_Degree)));
        float min_endY = (float) (centerY + radius * MINUTE_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(min_Degree)));
        float sec_endX = (float) (centerX + radius * SECOND_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(sec_Degree)));
    ...
    private void drawTimeNumbers(Canvas canvas) {
        ...
        float textHeight = fontMetrics.leading - fontMetrics.ascent;
        // 粗糙绘制
        for (int i = 0; i < 12; i++) {
...
//            Log.d("TAG", "hour: "+number+" textHeight: "+textHeight+" textWidth: "+textWidth);
            canvas.drawText(
                    number,
                    (float) (centerX + (radius * 0.78f ) * Math.cos(Math.toRadians(hourDegree))),
                    (float) (centerY + (radius * 0.78f ) * Math.sin(Math.toRadians(hourDegree))+textHeight*0.4f),
                    numberPaint
            );
        }
    }
